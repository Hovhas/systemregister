...
class Organization(Base):
    ...
    org_type: Mapped[OrganizationType] = mapped_column(SAEnum(OrganizationType), nullable=False)
...
class System(Base):
    ...
    system_category: Mapped[SystemCategory] = mapped_column(SAEnum(SystemCategory), nullable=False)
    ...
    nis2_classification: Mapped[NIS2Classification | None] = mapped_column(SAEnum(NIS2Classification))
    ...
    lifecycle_status: Mapped[LifecycleStatus] = mapped_column(
        SAEnum(LifecycleStatus), default=LifecycleStatus.ACTIVE
    )
...
class SystemOwner(Base):
    ...
    role: Mapped[OwnerRole] = mapped_column(SAEnum(OwnerRole), nullable=False)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
...
class SystemIntegration(Base):
    ...
    description: Mapped[str | None] = mapped_column(Text)
    criticality: Mapped[Criticality | None] = mapped_column(SAEnum(Criticality, name="integration_criticality"))
...
class GDPRTreatment(Base):
    ...
    processor_agreement_status: Mapped[ProcessorAgreementStatus | None] = mapped_column(
        SAEnum(ProcessorAgreementStatus)
    )
